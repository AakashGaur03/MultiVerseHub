...
const ParticularGame = () => {
	...
	const loaderTrue = useSelector((state) => state.games.gamesDataParticularState === "loading");
	...
	const particularGameData = useSelector((state) => state.games.gamesDataParticular);
	useEffect(() => {
	...
	return (
		<>
			{gameData ? (
				<div className={`container-md ${theme} ${textColor}`}>
					<div
						style={{
							background:
								theme === "black" || theme.includes("dark")
									? "linear-gradient(45deg, black, transparent)"
									: "linear-gradient(45deg, #f0f0f0, transparent)",
						}}
						className=""
					>
						<div className="row my-5">
								...
								<div className="">
									<div className="sticky top-24">
										<Card style={{ width: "auto", minHeight: "200px" }}>
											<ImageWithLoader src={gameData.thumbnail} alt="Game Thumbnail" failedImage="/ImageNotFound.png" />
										</Card>
							...
							<div className="col-md-8">
								<section>
									<h2 className="text-3xl mb-3 md:mt-0 mt-6 text-center">{gameData?.title}</h2>
									<div className="mb-3">
										{gameData?.release_date && <>Release Date : {formatDateinHumanredable(gameData?.release_date)}</>}
									</div>
									<div className={`italic ${textColor} mb-3`}>
										{gameData?.short_description ? gameData?.short_description : "Tagline Not Available"}
									</div>
									<div className="mb-2">Description</div>
									<div className="leading-6 px-6 mb-3">
										{gameData?.description ? gameData?.description : "Description not Available"}
									</div>
								...
								<section className="my-5">
									<div className="mb-5">
										<div className={`font-semibold text-2xl ${textColor} text-center`}>
											<span className="cursor-pointer">Minimum System Requirements</span>
										</div>
									</div>
									<div className="row">
										<div className="col-sm-6">
											{gameData?.minimum_system_requirements?.os && (
												<GameInfoComp label="OS" value={gameData?.minimum_system_requirements?.os} />
											)}

											{gameData?.minimum_system_requirements?.memory && (
												<GameInfoComp label="Memory" value={gameData?.minimum_system_requirements?.memory} />
											)}

											{gameData?.minimum_system_requirements?.storage && (
												<GameInfoComp label="Storage" value={gameData?.minimum_system_requirements?.storage} />
											)}
										</div>
										<div className="col-sm-6">
											{gameData?.minimum_system_requirements?.processor && (
												<GameInfoComp label="Processor" value={gameData?.minimum_system_requirements?.processor} />
											)}

											{gameData?.minimum_system_requirements?.graphics && (
												<GameInfoComp label="Graphics" value={gameData?.minimum_system_requirements?.graphics} />
											)}

											<GameInfoComp label=" Additional Notes" value="Specifications may change during development" />
										</div>
									</div>
								</section>
							</div>
					...
					<section className="my-5 mx-4">
						<div className="flex mb-5">
							<div className={`mb-2 mt-3 ps-5 font-semibold text-2xl ${textColor} text-center`}>
					...
					<section>
						...
						<div className="row mb-3 text-center">
							...
							<div className="col-6 col-md-4">
								<GameInfoComp label="Release Date" value={gameData?.release_date} />
							</div>
							<div className="col-6 col-md-4">
