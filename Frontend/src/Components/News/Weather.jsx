...
const Weather = () => {
	...
	const loaderTrue = useSelector((state) => state.weather.weatherStatus === "loading");
		...
		<>
			<div className="d-flex justify-content-center flex-col align-items-center mt-5">
				<div className="d-flex relative w-4/5">
					<input
						className={`form-control weatherInput ${bgColor} ${textColor} ${placeholderColor} border border-gray-400 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400`}
						type="search"
					...
					<Card className="myCard mt-3" style={{ maxWidth: "18rem", backgroundImage }}>
						<Card.Body className={getCardBodyClassName(condition.text)}>
							<div className="text-center">
								<img src={condition.icon} className="iconwidth card-img-top" alt="..." />
							</div>
							<Card.Title className="text-center textrotate mt-4">{location?.name?.toUpperCase()}</Card.Title>
							<Card.Title className="text-center textrotate mt-4">{condition?.text?.toUpperCase()}</Card.Title>
							<Row className="mt-4 frontSide">
								<Col md={6}>
									<div className="text-center" id="cityTemp">{`CURRENT TEMP : ${temp_c}`}</div>
								</Col>
								<Col md={6}>
									<div className="text-center" id="cityFeelsLike">{`FEELS LIKE : ${feelslike_c}`}</div>
								</Col>
								<Col md={6}>
									<div className="text-center mt-4" id="cityWindSpeed">{`WIND SPEED : ${gust_kph}`}</div>
								</Col>
								<Col md={6}>
									<div className="text-center mt-4" id="cityHumidity">{`HUMIDITY : ${humidity}`}</div>
								</Col>
							</Row>
							<Row className="mt-4 backSide">
								<Col md={6}>
									<div className="text-center textrotate" id="cityWindDirection">{`WIND DIRECTION : ${wind_dir}`}</div>
								</Col>
								<Col md={6}>
									<div
								...
								<Col md={6}>
									<div className="text-center textrotate mt-4" id="cityLatitude">{`LATITUDE : ${location.lat}`}</div>
								</Col>
								<Col md={6}>
									<div className="text-center textrotate mt-4" id="cityLongitude">{`LONGITUDE : ${location.lon}`}</div>
								</Col>
							</Row>
						</Card.Body>
					</Card>
